...
namespace AccurateAppend.Websites.Clients.Areas.Authentication.Google
{
    ...
    public class Controller : System.Web.Mvc.Controller
    {
        ...
        private readonly GoogleSecurityConfiguration configuration;
        ...
        public Controller(Accounting.DataAccess.DefaultContext context, GoogleSecurityConfiguration configuration, IFormsAuthentication fa, IMembershipService ms, IMessageSession bus)
        {
        ...
        public virtual async Task<ActionResult> DoPostback(CancellationToken cancellation)
        {
            ...
            if (logonResult.Type == ExternalLogonResult.Accepted)
            {
                ...
                var logon = await logons.FirstOrDefaultAsync(l => l.Identity == externalIdentifier && l.Provider == IdentityProvider.Google, cancellation);
                ...
                if (logon == null && !this.User.Identity.IsAuthenticated)
                {
                    return this.View(ViewPath, new LinkModel()
                    {
                        ...
                        DisplayName = name,
                        Postback = new MvcActionModel() { ActionName = "Link", AreaName = "Authentication", ControllerName = "Google" }
                    });
                }
                ...
                using (var transaction = new TransactionScope(TransactionScopeOption.Required, TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    if (logon == null)
                    {
                        var user = this.context.SetOf<User>().InteractiveUser().First();
                        logon = new MappedIdentityLogon(user, externalIdentifier, IdentityProvider.Google);
                ...
                if (!logon.User.IsLockedOut)
                {
                    ...
                    var principal = await this.ms.GetPrincipalAsync(logon.User.UserName, Core.IdentityModel.AuthenticationTypes.Google, cancellation);

                    if (!this.User.Identity.IsAuthenticated) this.fa.CreateAuthenticationToken(principal);

                    await this.context.SaveChangesAsync(cancellation);

                    if (Session["Social Redirect To"] != null)
                    {
                        var redirect = Session["Social Redirect To"].ToString();
                        Session.Remove("Social Redirect To");
            ...
            return this.DisplayErrorResult("Google did not provide us a meaningful reply when accessing your profile. Please attempt to logon again. If the problem persists, please contact customer support.");
        }

        public async Task<ActionResult> Link(LinkModel model, CancellationToken cancellation)
        {
            model.Postback = new MvcActionModel() { ActionName = "Link", AreaName = "Authentication", ControllerName = "Google" };
            ...
            try
            {
                var principal = await this.ms.GetPrincipalAsync(model.UserName, Core.IdentityModel.AuthenticationTypes.Google, cancellation);
                ...
                {
                    return this.DisplayErrorResult("We're sorry but this Google account is already associated with another Accurate Append user. Perhaps you have more than one account? You can always reset your Facebook permissions and start over from scratch. Please contact customer support for further guidance.");
                }
            ...
            catch (Exception ex)
            {
                ...
                return this.DisplayErrorResult("We're sorry but we were unable to link your Google account with this logon. Please contact customer support for further guidance.");
            }
            finally
            {
                this.configuration.FinalizeLogon(this.HttpContext);
