...
namespace AccurateAppend.Websites.Clients.Areas.Authentication.LinkedIn
{
    ...
    public class Controller : System.Web.Mvc.Controller
    {
        ...
        private readonly LinkedInSecurityConfiguration configuration;
        ...
        public Controller(Accounting.DataAccess.DefaultContext context, LinkedInSecurityConfiguration configuration, IFormsAuthentication fa, IMembershipService ms, IMessageSession bus)
        {
        ...
        public virtual async Task<ActionResult> DoPostback(CancellationToken cancellation)
        {
            ...
            if (logonResult.Type == ExternalLogonResult.Accepted)
            {
                ...
                var logon = await logons.FirstOrDefaultAsync(l => l.Identity == externalIdentifier && l.Provider == IdentityProvider.LinkedIn, cancellation);
                ...
                if (logon == null && !this.User.Identity.IsAuthenticated)
                {
                    return this.View(ViewPath, new LinkModel()
                    {
                        ...
                        DisplayName = name,
                        Postback = new MvcActionModel() { ActionName = "Link", AreaName = "Authentication", ControllerName = "LinkedIn" }
                    });
                }
                ...
                if (logon == null)
                {
                    ...
                    logon = new MappedIdentityLogon(user, externalIdentifier, IdentityProvider.LinkedIn);
                ...
                if (!logon.User.IsLockedOut)
                {
                    ...
                    var principal = await this.ms.GetPrincipalAsync(logon.User.UserName, Core.IdentityModel.AuthenticationTypes.LinkedIn, cancellation);
            ...
            return this.DisplayErrorResult("LinkedIn did not provide us a meaningful reply when accessing your profile. Please attempt to logon again. If the problem persists, please contact customer support.");
        }

        public async Task<ActionResult> Link(LinkModel model, CancellationToken cancellation)
        {
            model.Postback = new MvcActionModel() { ActionName = "Link", AreaName = "Authentication", ControllerName = "LinkedIn" };
            ...
            try
            {
                var principal = await this.ms.GetPrincipalAsync(model.UserName, Core.IdentityModel.AuthenticationTypes.LinkedIn, cancellation);
                ...
                {
                    return this.DisplayErrorResult("We're sorry but this Linkedin account is already associated with another Accurate Append user. Perhaps you have more than one account? You can always reset your Linkedin permissions and start over from scratch. Please contact customer support for further guidance.");
                }
            ...
            catch (Exception ex)
            {
                ...
                return this.DisplayErrorResult("We're sorry but we were unable to link your LinkedIn account with this logon. Please contact customer support for further guidance.");
            }
            finally
            {
                this.configuration.FinalizeLogon(this.HttpContext);
