...
namespace AccurateAppend.Websites.Clients.Areas.Authentication.Messages
{
    ...
    {
        public static async Task<Message> SendNewAccountNotifcationToClient(String email, IdentityProvider provider, String displayName, Guid applicationId)
        {
            ...
            using (var body = new TemplateEngine())
            {
                ...
                body.SetValue("provider", provider);
                body.SetValue("displayname", displayName);
                ...
                {
                    Subject = "We noticed a new sign in added to your Accurate Append account.",
                    Body = body.Run(),
                    IsHtml = true
                };
        ...
        {
            var resource = new FileResource(@"EmailTemplates\New_Linked_Logon.Html.config");
            return resource;
        }
