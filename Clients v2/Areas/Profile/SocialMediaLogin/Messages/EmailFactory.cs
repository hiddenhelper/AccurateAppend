...
{
    /// <summary>
    /// Factory to construct the email message that is to be used with alternate credential invites.
    /// </summary>
    public static class EmailFactory
    {
        public static async Task<Message> SendInvite(String email, String userName, DateTime expiration, String url)
        {
            ...
            using (var body = new TemplateEngine())
            {
                ...
                body.SetValue("email", email);
                body.SetValue("username", userName);
                body.SetValue("expiration", $"{expiration.ToBillingZone()} PST");
                body.SetValue("invitelink", url);
        ...
        {
            var resource = new FileResource(@"EmailTemplates\AlternateCredentialsInviteMessage.config");
            return resource;
        }
